...
public class Refinanciacion_inDAO {

	private static final String sql_anularRef = "update refinanciacion_in set estado = ? where n_pedido = ?";
	private static final String sql_modificarRef = "update refinanciacion_in set dias = ?, cuota_diaria = ?,"
			+ " estado = ? where n_pedido = ?";
	
	private static final String sql_buscar_ref = "select * from refinanciacion_in where n_pedido = ? and"
			+ " estado <> ?";
		
	private static final String sql_insertarref = "insert into refinanciacion_in("
			+ "n_pedido, dias, cuota_diaria, estado) values(?, ?, ?,?)";
	
	private static final Connection sqlConexion = Conexion.getConexion();
	
	
	public Refinanciacion_inVO buscarRef(int n_pedido) throws SQLException{
		
		Refinanciacion_inVO _refVO = new Refinanciacion_inVO();
	...
	public int modificarRef(Refinanciacion_inVO _refVO) throws SQLException{
	...
	public int insertarNuevaRef(Refinanciacion_inVO _refVO) throws SQLException{
